...
async function fetchLiveJobs(){
  try{
    const res=await fetch("/api/search",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5",max_tokens:1500,tools:[{"type":"web_search_20250305","name":"web_search"}],messages:[{role:"user",content:`Search for current open internship and analyst positions (Praktikum/Intern/Summer Analyst) at major investment banks, private equity firms, private credit/direct lending funds, and top consulting firms in Germany (Frankfurt/Munich), Switzerland (Zurich), and London in 2026 and 2027. Search across LinkedIn job postings, Xing Stellenmarkt, WiWi-Treff forum (wiwitreff.de), eFinancialCareers, and company career pages. Look for roles at Goldman Sachs, JPMorgan, Deutsche Bank, Rothschild, Lazard, Evercore, Blackstone, KKR, Carlyle, EQT, Ardian, Pemberton, Tikehau, Ares, McKinsey, BCG, Bain, Roland Berger. Return ONLY a valid JSON array (no markdown, no text outside the array) with up to 15 entries: [{"firm":"name","role":"exact title","type":"IB or PE or PC or MBB","loc":"city","region":"D or CH or LDN","year":2026,"deadline":"YYYY-MM-DD or null","url":"direct job url or careers page","source":"linkedin or xing or wiwitreff or careers or efinancialcareers","note":"1 sentence describing what is currently open"}]. Use region "D" for Germany, "CH" for Switzerland, "LDN" for London. Only include roles with clear current evidence.`}]})});
    ...
    <div style={{fontFamily:sans,background:"transparent"}}>
      ...
      <div style={{background:"#0a0a0d",borderBottom:`0.5px solid ${C.border}`,padding:"8px 12px",overflowX:isMobile?"auto":"visible"}}>
        ...
        <div style={{display:"flex",gap:6,minWidth:isMobile?"500px":"auto"}}>
          ...
          <KpiCard label="DEUTSCHLAND" value={nDACH} sub="FFM · MUC · BER"  color={C.text} onClick={()=>setFR(filterRegion==="D"?"ALL":"D")} active={filterRegion==="D"}/>
      ...
      <div style={{background:C.bg,borderBottom:`0.5px solid ${C.border}`,padding:"0 12px",display:"flex",justifyContent:"space-between",alignItems:"stretch",overflowX:isMobile?"auto":"visible"}}>
        <div style={{display:"flex",flexShrink:0}}>
          ...
          {[["ALL","ALLE"],["D","Deutschland"],["CH","ZRH"],["LDN","LDN"]].map(([v,l])=>(
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                <div>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2}}>
                    ...
                    <span style={{fontFamily:mono,fontSize:10,padding:"2px 6px",background:{DACH:C.blueDim,CH:C.amberDim,LDN:C.greenDim}[job.region]||C.orangeDim,color:{D:C.blue,CH:C.amber,LDN:C.green}[job.region]||C.orange}}>{job.region==="D"?"D (FFM/MUC)":job.region==="CH"?"ZÜRICH":"LONDON"}</span>
